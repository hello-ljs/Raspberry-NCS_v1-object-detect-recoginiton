...
prison_path='/home/sdu/main/tmp_ncs/prison'
number_path='/home/sdu/main/tmp_ncs/number'
# ***************************************************************
...
def imwrite_cut_img(display_image, object_info, c, fna):
    ...
    if (percentage <= min_score_percent):
        # ignore boxes less than the minimum score
        return
    if (class_id==1):#if the box is cups ,then break
        return 
    if (class_id==2):# class_id =2 is persioners
...
# network expectations and return it
    ...
    # cv2.imwrite(outimg_path+'.jpg',roi_img)
        name = number_path + fna + str(c) + '.jpg'
        cv2.imwrite(name, roi_img)
