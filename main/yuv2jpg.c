...
int main(int argc,char *argv[])
{
    ...
    char *src_img;
    char *dst_img;
    static const char *optString = "w:h:i:o:";
	...
	unsigned char* pData = (unsigned char*)malloc(w * h*3/2);//存放yuv420sp数据
	unsigned char* rData = (unsigned char*)malloc(w*h*3/2);//存放yuv420p数据
	
	unsigned long dwSize = 0;
	FILE *rfp = fopen(src_img,"rb");
	if(NULL == rfp)
		printf("fopen file error!\n");

	fread(rData,w*h*3/2,1,rfp);	//读取yuv文件数据到rData里面
	yuv420p_to_yuv420sp(rData,pData,w,h);

	yuv420p_to_jpeg(dst_img,pData,w,h,100);
